...
with st.sidebar:
    ...
    hft_mode = st.checkbox("🚀 High-Frequency Mode", 
                          help="Enable maximum speed trading (10x faster order generation)")
    if hft_mode:
        st.info("⚡ HFT Mode: Orders generated every 50ms, batch processing enabled")
    ...
    with col1:
        if st.button("Start Simulation", disabled=st.session_state.simulation_running):
            if symbols:
                st.session_state.traders = create_traders(num_traders, symbols, initial_cash, hft_mode)
                start_simulation()
                if hft_mode:
                    st.success("🚀 HFT Simulation started! Maximum performance mode enabled.")
                else:
...
if st.session_state.simulation_running:
    # Auto-refresh for HFT speed (500ms)
    time.sleep(0.5)
    st.rerun()
