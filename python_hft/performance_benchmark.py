...
class PerformanceBenchmark:
    ...
    def start_benchmark(self, duration_seconds=60):
        ...
        print(
            f"Symbols: {self.traders[0].symbols if self.traders else 'None'}")
        print("-" * 50)
    ...
    def _print_realtime_stats(self, elapsed, stats):
        ...
        print(
            ...
            f"Latency: {self.avg_latency:5.2f}ms",
            end="",
            flush=True)
    ...
    def _print_final_results(self):
        ...
        print(
            f"   Total Volume:         {final_stats['total_volume']:,} shares")
        print(
            f"   Average TPS:          {final_stats['trades_per_second']:.1f}")
        print(f"   Peak TPS:             {self.peak_tps:.1f}")
        print(
            f"   Average Latency:      {final_stats['avg_latency_ms']:.2f} ms")
        ...
        if elapsed_time > 0:
            ...
            print(
                f"   Microseconds/Trade:   {(elapsed_time * 1000000) / max(1, final_stats['total_trades']):.0f} μs"
            )
